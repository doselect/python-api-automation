"""Mirrors tests/test_regression/test_hacker/test_attempt_dba.py (do-api-automation)."""
...
def test_attempt_dba(
    ...
    res2 = recruit_response_handler.get_list_of_problems(test_slug, "MAR", "DBA", 1)
    ...
    res19 = hacker_response_handler.post_create_solution(
        "DBA", candidate_username, problem_slug, test_slug, solutionset_id,
        problem_stub=problem_stub, solution_lang=solution_lang,
    )
    solution_slug = res19["solution_slug"]
    solution_id = res19["solution_id"]

    hacker_response_handler.post_code_run(
        "DBA", solution_lang, solution_code, problem_id, solution_id, sock_id, candidate_username
    )

    hacker_response_handler.patch_solution(
        solution_slug, "DBA", test_slug, solution_code=solution_code, solution_lang=solution_lang
    )
