"""Mirrors tests/test_regression/test_hacker/test_attempt_fib.py (do-api-automation)."""
...
def test_attempt_fib(
    ...
    res2 = recruit_response_handler.get_list_of_problems(test_slug, "MAR", "FIB", 1)
    ...
    res17 = hacker_response_handler.post_create_solution(
        "FIB", candidate_username, problem_slug, test_slug, solutionset_id, correct_answer=correct_answer
    )
    solution_slug = res17["solution_slug"]

    hacker_response_handler.patch_solution(solution_slug, "FIB", test_slug, correct_answer=correct_answer)
